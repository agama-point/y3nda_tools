...
class VersionAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        terminal = Terminal()
        print(terminal.color("y", "ycct.py {0} (Python 3.6+)".format(__version__)))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Show ycct.py and submodule versions.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
def print_verbose(args: argparse.Namespace, message: str, level: int = 1) -> None:
    """Print optional diagnostics without changing the command's standard output."""
...
def main() -> int:
    ...
    args = parse_args()
    print_verbose(args, "ycct.py {0} invoked.".format(__version__))
    if args.verbose >= 2:
        operation = args.cipher or ("all" if args.all else "none")
        print_verbose(args, "Selected operation: {0}.".format(operation), level=2)
    if args.mnemonic is not None:
        print_verbose(args, "Looking up a mnemonic entry.")
        print_mnemonic_lookup(args.mnemonic)
        return 0
    if args.net:
        print_verbose(args, "Running the network check.")
        print_network_info()
        return 0
    ...
    print_verbose(args, "Configuration file: {0}".format(args.config))
    print_verbose(args, "Project directory: {0}".format(project_directory))
    print_verbose(args, "Project logging: {0}.".format("enabled" if config["log"] else "disabled"), level=2)

    with console_log(project_directory, Path(__file__).name, bool(config["log"])):
        if args.status:
            print_verbose(args, "Showing project and system status.")
            terminal = Terminal()
        ...
        try:
            if args.run is not None:
                ...
                print_verbose(args, "Running flow file: {0}".format(flow_path))
                return flow_runner.run(flow_path, flow_runner.load(flow_path))
            if args.dump:
                print_verbose(args, "Dumping data file: {0}".format(project_directory / "data.txt"))
                try:
                ...
                hexdump(data)
                return 0
            input_source = "command-line text" if args.text is not None else str(project_directory / "data.txt")
            print_verbose(args, "Input source: {0}".format(input_source))
            input_text = args.text if args.text is not None else read_data(project_directory / "data.txt")
            if args.all:
                print_verbose(args, "Running all applicable conversions.")
            ...
            if args.cipher == "xor":
                ...
                print_verbose(args, "Applying Polybius transformation.")
                result = polybius_transform(input_text)
            else:
                print_verbose(args, "Applying leetspeak transformation.")
